...
export default function MyPreSet() {
    ...
    // ✅ userId 제거 - 백엔드에서 토큰으로 처리
    const handleShare = async (preset) => {
        if (!preset.presetId) {
            alert("프리셋 정보를 찾을 수 없습니다.");
            return;
        }

        if (!window.confirm(`'${preset.presetName}' 레시피를 공유하시겠습니까?`)) {
            return;
        }

        try {
            // ✅ presetId만 전송 (userId는 백엔드가 토큰에서 추출)
            ...
            if (error.response?.status === 409) {
                ...
                alert(error.response?.data || "공유 처리 중 오류가 발생했습니다.");
            }
        }
    };
...
// 삭제 핸들러 수정
    const handleDelete = async (presetId, postedUserId) => {
        ...
        try {
            // ✅ 작동 방식: await를 사용하여 서버의 응답을 기다립니다.
            const response = await deletePreset(presetId); // 보통 백엔드에서 토큰으로 처리하므로 presetId만 보내도 됩니다.
            
            if (response.status === 200 || response.status === 204) {
                alert("성공적으로 삭제되었습니다.");
                // ✅ 상태 업데이트: 화면에서 즉시 사라지게 합니다.
